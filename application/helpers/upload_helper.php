...
function handle_project_attachments_array($taskid, $index_name = 'attachments')
{
    $hookData = hooks()->apply_filters('before_handle_task_attachments_array', [
        'task_id' => $taskid,
        'index_name' => $index_name,
    ...
    $uploaded_files = [];
    $path           = get_upload_path_by_type('projecttask') . $taskid . '/';
...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
		...
		break;
        case 'projecttask':
            $path = PROJECT_TASK_ATTACHMENTS_FOLDER;
