...
class Invoice_view extends App_Controller
{
    ...
    public function index($id = '', $hash = '')
    {
        ...
        $title = 'Invoice ' . (isset($invoice['invoice_number']) ? $invoice['invoice_number'] : '');

        // Output HTML directly to avoid App_Loader issue
        $this->outputInvoiceHtml($invoice, $items, $payments, $bank_details, $company, $title, $hash);
    }

    private function outputInvoiceHtml($invoice, $items, $payments, $bank_details, $company, $title, $hash)
    {
        ?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .btn-download, .btn-print { display: none; }
        }
    </style>
</head>
<body>
        ...
        <div class="invoice-header">
            <div class="row">
                ...
                <div class="col-sm-6 text-right">
                    ...
                    <br>
                    <strong>Date:</strong> <?php echo !empty($invoice['invoice_date']) ? date('d M Y', strtotime($invoice['invoice_date'])) : ''; ?><br>
                ...
                <div class="col-sm-6">
                    <div class="totals-box">
                        ...
                        <div class="total-row">
                            ...
                            <span><?php echo number_format((float)($invoice['tax_amount'] ?? 0), 2); ?></span>
                        </div>
                        <div class="total-row">
                            <span class="total-label text-success">Paid:</span>
                            <span class="text-success"><?php echo number_format((float)($invoice['paid_amount'] ?? 0), 2); ?></span>
                        </div>
                            ...
                            <tbody>
                                <?php foreach ($payments as $payment) { ?>
                                    <tr>
                                        ...
                                        <td class="text-success"><strong><?php echo number_format((float)($payment['amount'] ?? 0), 2); ?></strong></td>
    ...
    public function pdf($id = '', $hash = '')
    {
        ...
        $invoiceNumber = isset($invoice['invoice_number']) ? $invoice['invoice_number'] : $id;
        $filename = 'invoice-' . slug_it($invoiceNumber) . '-' . $id;

        if (ob_get_length()) {
            ob_end_clean();
        }

        $pdf->Output(mb_strtoupper($filename) . '.pdf', 'D');
    }
}
