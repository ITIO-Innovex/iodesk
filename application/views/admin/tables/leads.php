...
$rules = [
	...
	App_table_filter::new('company_id', 'SelectRule')->label(_l('Company Name'))->options(function ($ci) {
        return collect($ci->leads_model->get_compname())->map(fn ($company) => [
...
return App_table::find('leads') 
    ->outputUsing(function ($params) use ($statuses,$pagexxx) {
		...
		//array_push($where, ' AND (company_id = 2) ');
