...
return App_table::find('invoices')
    ->outputUsing(function ($params) {
        ...
        $where  = [];
		 ...
		 if(!is_super()){
        array_push($where, 'AND ' . db_prefix() . 'invoices.company_id=' . $this->ci->db->escape_str(get_staff_company_id()));
        }else{
  
if(isset($_SESSION['super_view_company_id'])&&$_SESSION['super_view_company_id']){
array_push($where, 'AND ' . db_prefix() . 'invoices.company_id=' . $this->ci->db->escape_str($_SESSION['super_view_company_id']));
 }
 ...
 $aColumns = hooks()->apply_filters('invoices_table_sql_columns', $aColumns);
        ...
        foreach ($rResult as $aRow) {
            ...
            if (is_numeric($clientid) || $project_id) {
                $numberOutput = '<a XXXX href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" target="_blank">' . e(format_invoice_number($aRow['id'])) . '</a>';
            } else {
                $numberOutput = '<a TTTT href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" onclick="init_invoicex(' . $aRow['id'] . '); return false;">' . e(format_invoice_number($aRow['id'])) . '</a>';
            }
