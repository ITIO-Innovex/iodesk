...
<div class="row">
<?php
    $statuses = $this->project_model->get_proj_status();
	
	//print_r($statuses);exit;
	//print_r($default_tickets_list_statuses);exit;
?>
...
foreach ($statuses as $status) {
    $_where = '';
    if ($where == '') {
        $_where = 'project_status=' . $status['id'];
    } else {
        $_where = 'project_status=' . $status['id'] . ' ' . $where;
    }
    if (isset($project_id)) {
        $_where = $_where . ' AND project_id=' . $project_id;
    }
   $_where = $_where . ' AND company_id= '.get_staff_company_id();  //AND status=1 ?>
    <div class="col-md-2 col-xs-6 md:tw-border-r md:tw-border-solid md:tw-border-neutral-300 last:tw-border-r-0">
        <a href="#" data-cview="ticket_status_<?php echo e($status['id']); ?>"
            ...
            <span class="tw-font-semibold tw-mr-3 rtl:tw-ml-3 tw-text-lg">
                <?php echo total_rows(db_prefix() . 'project_master', $_where); ?>
            </span>
            <span style="color:<?php echo e($status['color']); ?>">
                <?php echo proj_status_translate($status['id']); ?>
            </span>
        </a>
    </div>
    <?php
