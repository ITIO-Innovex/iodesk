    ...
    <div class="row">
      <div class="col-md-12 tw-mb-8">
        <h4 class="tw-mt-0 tw-font-semibold tw-text-lg tw-text-neutral-700 tw-ml-2.5 tw-inline-flex tw-items-center">
          <i class="fa-solid fa-chart-pie tw-mx-2"></i> Leads by Stage Report
        </h4>
        ...
        <div class="panel_s">
          <div class="panel-body">
            <h4 class="tw-mt-0 tw-font-semibold tw-text-md tw-text-neutral-700 tw-ml-2.5 tw-inline-flex tw-items-center">
              <i class="fa-solid fa-chart-simple tw-mx-2"></i> Stage Distribution
            </h4>
            <div class="row">
              <div class="col-md-6">
        ...
        <div class="panel_s">
          <div class="panel-body">
            <h4 class="tw-mt-0 tw-font-semibold tw-text-md tw-text-neutral-700 tw-ml-2.5 tw-inline-flex tw-items-center">
              <i class="fa-solid fa-table tw-mx-2"></i> Leads by Stage Details
            </h4>
            <?php if (isset($leads_by_stage_data) && count($leads_by_stage_data) > 0) { ?>
            <table class="table dt-table" data-order-col="1" data-order-type="desc">
              ...
              <tbody>
                <?php 
                $total_leads = 0;
                foreach ($leads_by_stage_data as $stage) {
                    $total_leads += $stage['total'];
                }
                ?>
                <?php foreach ($leads_by_stage_data as $stage) { 
                    $percentage = ($total_leads > 0) ? round(($stage['total'] / $total_leads) * 100, 2) : 0;
                ?>
                <tr>
                  <td>
                    <a href="<?php echo admin_url('leads?status=' . $stage['status_id']); ?>" target="_blank">
                      <?php echo e($stage['status_name']); ?>
                    </a>
                  </td>
                  <td>
                    <span class="tw-font-semibold"><?php echo e($stage['total']); ?></span>
                  </td>
                  <td>
                  ...
                  <td>
                    <a href="<?php echo admin_url('leads?status=' . $stage['status_id']); ?>" 
...
<script type="text/javascript">
  ...
  function drawPieChart() {
    var data = google.visualization.arrayToDataTable([
      ['Stage', 'Leads'],
      <?php 
      if (isset($leads_by_stage_data) && count($leads_by_stage_data) > 0) {
          foreach ($leads_by_stage_data as $stage) {
              echo "['" . addslashes($stage['status_name']) . "', " . $stage['total'] . "],";
          }
      }
      ?>
    ]);
    ...
    var options = {
      title: 'Leads Distribution by Stage',
      pieHole: 0.4,
      colors: [
        <?php 
        if (isset($leads_by_stage_data) && count($leads_by_stage_data) > 0) {
            foreach ($leads_by_stage_data as $stage) {
                echo "'" . $stage['status_color'] . "',";
            }
        }
        ?>
      ],
  ...
  function changePeriod(period) {
    window.location.href = '<?php echo admin_url('reports/leads_by_stage'); ?>?period=' + period;
  }
</script>
