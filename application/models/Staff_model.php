...
class Staff_model extends App_Model
{
  ...
  // New condation for superadmin admin and staff
	...
	public function addcompany($data)
    {
        ...
        $this->db->insert(db_prefix() . 'company_master', $data);
        $company_id = $this->db->insert_id();
        if ($company_id) {
            log_activity('New Company Added [ID: ' . $company_id . ', ' . $data['companyname'] . ' ' . $data['website'] . ']');
           // hooks()->do_action('staff_member_created', $staffid);
            return $company_id;
        }
	...
	public function getcompany($id = '', $where = [])
    {
        //$select_str = '*,CONCAT(firstname,\' \',lastname) as full_name';

        // Used to prevent multiple queries on logged in staff to check the total unread notifications in core/AdminController.php

		if (!is_admin()) $fetch_via_staff_id=" AND staffid='" . get_staff_user_id()."'";
		else $fetch_via_staff_id="";
       ...
       // $this->db->select($select_str);
        ...
        if (is_numeric($id)) {
            ...
            $staff = $this->db->get(db_prefix() . 'company_master')->row();
            return $staff;
        }
		//$this->db->where_not_in('company_id', 1);
        //$this->db->order_by('companyname', 'desc');
        //return $this->db->get(db_prefix() . 'company_master')->result_array();
		
		
		//$this->db->select('s.firstname, s.lastname, cm.companyname');
		$this->db->select('cm.*, COUNT(s.staffid) as staff_count');
        $this->db->from(db_prefix() . 'company_master cm');
        $this->db->join(db_prefix() . 'staff s', 's.company_id = cm.company_id', 'left');
        $this->db->where_not_in('cm.company_id', [1]);
        $this->db->group_by('cm.company_id');
        $this->db->order_by('cm.companyname', 'desc');
	...
	public function updatecompany($data, $id)
    {
	   ...
	   if(isset($data['customize_company_name'])&&$data['customize_company_name']){
	   unset($data['customize_company_name']);
	   }
	   ...
	   unset($data['customize_company_domain']);
		...
		//echo $this->db->last_query();exit;
			if($this->db->affected_rows() > 0){
			log_activity('Update Added [ID: ' . $id . ', ' . $data['companyname'] . ' ' . $data['website'] . ']');
			return "Success";
			}else{
			return "Failed";
			}
	...
	public function deletecompany($id,$sid)
    {
		...
		//echo $this->db->last_query();exit;
			if($this->db->affected_rows() > 0){
			log_activity('Company Status Changed [ID: ' . $id . ', ' . $data['updatedon'] . ']');
			return "Success";
			}else{
			return "Failed";
			}
