...
class Leads_model extends App_Model
{
  ...
  $data['last_status_change']=date('Y-m-d H:i:s');
    ...
    public function get_source($id = false)
    {
	 ...
	 public function get_compname($id = false)
    {
        if (is_numeric($id)) {
		    $this->db->select('company_id, companyname');
            $this->db->where('company_id', $id);

            return $this->db->get(db_prefix() . 'company_master')->row();
        }
...
////////////////////////// UW Status /////////////////////////	
	 public function get_uw_status($id = '', $where = [])
     {
		...
		$this->db->where(db_prefix() . 'leads.company_id', get_staff_company_id());	// Use condition
		$this->db->where(db_prefix() . 'leads.assigned', $_SESSION['staff_logged_in']);	// Use condition
		}elseif(!is_admin() && get_staff_rolex()==4) { 
		$this->db->where(db_prefix() . 'leads.company_id', get_staff_company_id());	// Use condition
		}elseif(is_super()){
		
		if(isset($_SESSION['super_view_company_id'])&&$_SESSION['super_view_company_id']){
		$this->db->where(db_prefix() . 'leads.company_id', $_SESSION['super_view_company_id']);	// Use condition
		}
...
////////////////////////// End UW Status /////////////////////////	
	...
	//print_r($data);echo $id;exit;
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Task Status Updated [StatusID: ' . $id . ', Name: ' . $data['name'] . ']');
	...
	public function getAdminAllDepartments($adminCompanyId){
        $this->db->where('company_id', $adminCompanyId);
...
foreach ($data as $key => $value) {
	...
	if(isset($data['target_countries'])&&$data['target_countries']){
	...
	$data['last_status_change']= date('Y-m-d H:i:s');
        ...
        $data_clients['leadid']=$data['deal_id'];
        $data_clients['company_id']=get_staff_company_id();
		} 
   ...
   public function getLeadFormComapnyID($key){
        $this->db->select('company_id');
        $this->db->where('form_key', $key);
        $result = $this->db->get(db_prefix() . 'web_to_lead')->row();
        if($result){
...
$diffInHours = round($diffInSeconds / 3600);
 ...
 return '<i class="fa-solid fa-clock text-warning" title="In Progress"></i>';
	...
	public function get_deal_form_order($cid = '')
    {
		if(isset($cid)&&$cid){
