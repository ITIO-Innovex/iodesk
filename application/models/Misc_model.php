...
class Misc_model extends App_Model
{
	 ...
	 public function get_notes_home()
    {
	   ...
	   if (!is_super()) {
		$this->db->where('staff.company_id',get_staff_company_id()); 
		}else{
    ...
    public function get_user_notifications($read = false)
    {
		...
		elseif (is_admin()) 
		{
			$sql = 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread=' . $read. ' AND company_id=' . get_staff_company_id();
			$sql .= ' UNION ALL ';
			$sql .= 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread_inline=' . $read. ' AND fromuserid!=' . $staff_id. ' AND company_id=' . get_staff_company_id();
		}
		else
		{
			$sql = 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread=' . $read . ' AND touserid!=' . $staff_id. ' AND company_id=' . get_staff_company_id();
			$sql .= ' UNION ALL ';
			$sql .= 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread_inline=' . $read . ' AND fromuserid!=' . $staff_id. ' AND touserid=' . $staff_id. ' AND company_id=' . get_staff_company_id();		
		}
    ...
    public function _search_contacts($q, $limit = 0, $where = '')
    {
        ...
        if ($have_assigned_customers || $have_permission_customers_view || $tickets_contacts) {
			...
			if(!is_super()){
				$this->db->where('contacts.company_id',get_staff_company_id()); 
			}
